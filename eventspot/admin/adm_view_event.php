...
if (isset($_POST)) {
    if (count($_POST) > 0) {
        if (isset($_POST['refused'])) {
            $sql = "UPDATE event SET status = 3 WHERE idEvent = ?";
        } elseif (isset($_POST['approved'])) {
            $sql = "UPDATE event SET status = 1 WHERE idEvent = ?";
        }
        ...
        <div class="card w-100 border-0 box shadow-sm rounded-4">
            <div class="card-body m-1 p-1">
                ...
                <?php if ($record['status'] == 1) : ?>
                    <div class="card-body m-1 p-1 d-flex justify-content-end">
                        <button class="btn btn-success me-1" disabled>Aprovado</button>
                    </div>
                <?php elseif ($record['status'] == 2) : ?>
                    <div class="card-body m-1 p-1 d-flex justify-content-end">
                        <form action="" method="post">
                            <button class="btn btn-outline-success me-1" type="submit" name="approved">Aprovar</button>
                            <button class="btn btn-outline-danger ms-1" type="submit" name="refused">Reprovar</button>
                        </form>
                    </div>
                <?php elseif ($record['status'] == 3) : ?>
                    <div class="card-body m-1 p-1 d-flex justify-content-end">
                        <button class="btn btn-danger me-1" disabled>Reprovado</button>
                    </div>
                <?php endif ?>
            </div>
        <?php endforeach ?>
